    ...
    try:
        ...
        if request.method ==  'POST':    
            ...
            if not utils.isUsernameValid(username):
                error = "El Nombre de Usuario debe ser alfanumerico '.','_','-'"
                flash(error)
                return render_template('auth/register.html') 
            ...
            if db.execute('SELECT * FROM user WHERE email = ?', (email,)).fetchone() is not None:
                error =  'El correo electronico {} ya esta registrado.'.format(email)
                flash(error)
                return render_template('auth/register.html')
            
            if (not utils.isPasswordValid(password)):
                error = 'La contraseña debe contener como minimo 8 caracteres, al menos una letra minúscula, una letra mayúscula y un número'
                flash(error)
                return render_template('auth/register.html')
...
def confirm():
    try:
        ...
        if request.method == 'POST': 
            ...
            if  not password:
                flash('Se requiere la contraseña')
                return render_template('auth/change.html', number=authid)

            if not password1:
                flash('Se requiere la confirmación de la contraseña')
                return render_template('auth/change.html', number=authid)
            ...
            if attempt is not None:
                ...
                db.commit()
                flash('Su contraseña fue moficiada exitosamente!')
                return redirect(url_for('auth.login'))
            else:
                flash('Invalid')
                return render_template('auth/forgot.html')
...
def forgot():
    try:
        ...
        if request.method == 'POST':
            ...
            if (not email or (not utils.isEmailValid(email))):
                error = 'Dirección de Correo Electronica no valida'
                flash(error)
                return render_template('auth/forgot.html')
...
def login():
    try:
        ...
        if request.method == 'POST':
            ...
            if  not username:
                error = 'El Nombre de Usuario es un campo requerido'
                flash(error)
                return render_template('auth/login.html')

            if  not password:
                error = 'La contraseña es un campo requrido'
                flash(error)
            ...
            if  user is None:
                error = 'El Nombre de Usuario o la Contrasela es incorrecta'
