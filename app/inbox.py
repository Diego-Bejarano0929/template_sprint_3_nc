...
def show():
    ...
    messages = db.execute(
        ...
        'SELECT message.from_id , message.to_id , message.created , message.subject, message.body, user.username AS user_from, u.username AS user_to FROM message INNER JOIN "user" ON message.from_id = user.id INNER JOIN "user"AS u ON message.to_id = u.id WHERE to_id = ? OR from_id = ?',(g.user['id'], g.user['id'])
